...
export default function NewOtpVerificationScreen() {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const response = await fetch(`${environment.apiBaseUrl}/auth/signup/verify-otp`, {
        method: 'POST',
        headers: {
      ...
      router.push({ 
        pathname: '/auth/new-password-setup', 
        params: { 
          mobileNumber: mobileNumber as string,
          userId: data.userId,
        } 
  ...
  const handleResendOtp = async () => {
    ...
    setIsResending(true);
    try {
      const response = await fetch(`${environment.apiBaseUrl}/auth/signup/initial`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
